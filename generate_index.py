...
- Preferiti e Visti recentemente nella tendina principale
...
- InfoCard con trailer incorporato (40% della card)
- Poster come sfondo della InfoCard
- Trailer automatico da TMDB (YouTube)
"""
...
<div id='infoCard'>
  <div>
    <h2 id="infoTitle"></h2>
    <iframe id="infoTrailer" src="" allow="autoplay; fullscreen" allowfullscreen></iframe>
...
function openInfo(item, push=true) {{
    ...
    let color="#4caf50";
    if(vote<5){{ color="#f44336"; }} else if(vote<7){{ color="#ff9800"; }}
    infoVote.innerHTML=`<svg viewBox="0 0 36 36" class="circular-chart">
        <path class="circle-bg" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"/>
        <path class="circle" stroke="${{color}}" stroke-dasharray="${{dash}},100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"/>
        <text x="18" y="20.35" class="percentage">${{vote}}</text></svg>`;
    ...
    infoTrailer.src = item.trailer || ""; // trailer automatico

    playBtn.onclick=()=> {{
        if(item.trailer) {{
            infoTrailer.src=item.trailer;
        }} else {{
            let link=item.link;
            if(item.type==='tv') {{
...
// Funzioni render, toggleFavorite, addToRecent, populateGenres, updateType rimangono come nel tuo script originale
...
def main():
    ...
    for type_, url in SRC_URLS.items():
        ...
        for idx, tmdb_id in enumerate(ids):
            ...
            # Trailer automatico da TMDB (YouTube)
            trailer = ""
            if info.get("videos") and info["videos"].get("results"):
                for v in info["videos"]["results"]:
                    if v["type"].lower() == "trailer" and v["site"].lower() == "youtube":
            ...
            entries.append({
                ...
                "overview": overview,
                "link": link,
                "trailer": trailer,       # trailer incluso
                "type": type_,
                "seasons": seasons,
                "episodes": episodes,
    ...
    # Unione con archivio precedente
