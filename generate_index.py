...
closeCardBtn.onclick = () => {{
  ...
  history.replaceState({{page:"grid"}}, "", "#grid"); // reset su griglia
}};
...
function openPlayer(item, push=true) {{
    ...
    let link = item.type==='tv'
        ? `https://vixsrc.to/tv/${{item.id}}/1/1?lang=it&autoplay=1`
        : `https://vixsrc.to/movie/${{item.id}}/?lang=it&autoplay=1`;
    iframe.src = link;
    if(push) {{
